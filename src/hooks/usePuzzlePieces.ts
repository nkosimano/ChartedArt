...
import { PuzzlePiece, PuzzlePieceStatus } from '@/components/puzzle';
...
export const usePuzzlePieces = (movementId: string) => {
  ...
  const fetchPieces = useCallback(async () => {
    try {
      ...
      const formattedPieces: PuzzlePiece[] = (data || []).map((piece: PuzzlePieceData) => ({
        id: piece.id,
  ...
  useEffect(() => {
    if (movementId) {
      ...
      // Set up real-time subscription for piece updates
...
export const useReservePuzzlePiece = () => {
  ...
  const reservePiece = useCallback(async (pieceId: string): Promise<Reservation> => {
    try {
      ...
      // Call backend API to reserve the piece
